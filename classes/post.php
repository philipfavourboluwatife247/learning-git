...
class Post
{
			...
			if($this->error == ""){
				...
				if(isset($data['parent']) && is_numeric($data['parent'])){
					...
					if(is_array($mypost) && $mypost['userid'] != $userid){
						...
						//add notification
						add_notification($_SESSION['palsworld_userid'],"comment",$mypost);
					}
	...
	public function i_own_post($postid,$palsworld_userid)
	{
		...
		$result = $Database->read_from_db($query);
  		...
  		if(is_array($result)){
  			...
  			if($result[0]['userid'] == $palsworld_userid){
	...
	public function like_post($id,$type,$palsworld_userid){
		...
		$Database = new Automated_DB();
			...
			if(is_array($result)){
				...
				if(!in_array($palsworld_userid, $user_ids)){

					$arr["userid"] = $palsworld_userid;
					...
					if($type != "user"){
						...
						//add notification
						add_notification($_SESSION['palsworld_userid'],"like",$single_post);
					}

				}else{

					$key = array_search($palsworld_userid, $user_ids);
					unset($likes[$key]);
